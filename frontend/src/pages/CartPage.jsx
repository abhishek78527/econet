...
import { FiTrash2, FiShoppingBag, FiArrowLeft, FiShield, FiTruck } from 'react-icons/fi'
...
export default function CartPage() {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      const { data } = await api.post('/orders', { items, address, payment_mode: 'COD' })
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '72px 20px 40px', position: 'relative', zIndex: 1 }}>
      ...
      {!cart.items?.length ? (
        <div className="card" style={{ padding: '64px 24px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--t3)', marginBottom: 20 }}>Add some products to get started</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, alignItems: 'start' }}>
          ...
          <div className="card au1" style={{ padding: 22, position: 'sticky', top: 72 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 10, display: 'flex', justifyContent: 'space-between', fontSize: 16, fontWeight: 800, color: 'var(--t1)' }}>
                <span>Total</span>
                <span style={{ color: '#34d399', fontFamily: 'DM Mono' }}>₹{(total + (total > 499 ? 0 : 49)).toFixed(0)}</span>
              </div>
            </div>
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: 'block', fontSize: 12, color: 'var(--t3)', fontWeight: 600, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.5px' }}>Delivery Address</label>
              <textarea className="input" rows={3} placeholder="Enter your full delivery address…" value={address} onChange={e => setAddress(e.target.value)} style={{ resize: 'none', fontSize: 13 }}/>
            </div>
            <button onClick={placeOrder} disabled={placing || !cart.items.length} className="btn-primary" style={{ width: '100%', padding: '13px', fontSize: 15, fontWeight: 700, borderRadius: 12 }}>
